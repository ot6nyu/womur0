# USAGE
#frameworkpython3 ./budz_cnn_predictor6.py --shape-predictor ./budz_predictor_landmarks.dat --video ../../face_recognition-master/examples/bbunny-clip0.mp4 --output ./budz_predictor0.avi
...
def frameincrement(frame_number):
    ...
    frame_number += 1
    print("grab frame, frame# = {}".format(frame_number))
    # if not grabbed, then no more stream
    return grabbed, frame, frame_number

def dispwrite(writer, args, frame, fps):
...
#         label = recognize_face(embedding, embeddings, labels)
    return shape
# def drawfeat(shape, leyepos, reyepos, nosepos, frame):
def drawfeat(shape, frame):
    ...
    for e in range(1000):
        ...
        print("lcolor {}".format(lcolor[len(lcolor) - 1]))
        for i, pos in enumerate(leyepos):
            x1, y1 = list(pos)
#             print("x1: {}, y1: {}".format(x1, y1))
#             cv2.circle(frame, (x1, y1), 1, lcolor[i], -1)
            if ((x1 - e1cx) ** 2) / (r2 ** 2) + ((y1 - int(shape[1][1])) ** 2) / (r1 ** 2) <= 1 :
#                 cv2.circle(frame, pos, randint(1, 5), lcolor[len(lcolor) - 1], -1)
                cv2.circle(frame, pos, randint(1, 5), lcolor[randint(0, len(lcolor) - 1)], -1)
...
while True:
    ...
    if len(faces) == 0:
        print("no face detected, frame to write: frame# {}".format(frame_number))
        frame, writer = dispwrite(writer, args, frame, fps)
        grabbed, frame, frame_number = frameincrement(frame_number)
    else:
    ...
    #         d = int(r * 1.414)
            ...
            thickness = 1
            cv2.ellipse(frame, center, axes, rotation_angle, start_angle, end_angle, color, thickness)
            shape = face_shaper(rgb_img, face)
            frame = drawfeat(shape, frame)
#             for p in range(100):
...
#                     print("dots: {}".format(dots))
        print("face scribbled with {} points, frame to write: frame# {}".format(len(nosepos), frame_number))
        frame, writer = dispwrite(writer, args, frame, fps)
        grabbed, frame, frame_number = frameincrement(frame_number)
#  save output image
